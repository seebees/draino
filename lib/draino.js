var util = require('util'),
    ...
    tell = function (toEvent, accumulation, error, cares) {
        //if we have buffers send the array, otherwise be nice and .join('')
        accumulation = Buffer.isBuffer(accumulation[0]) ? 
            accumulation : 
            accumulation.join('');
    ...
    engine = function (
        ...
        waiting,            //things waiting for accumulation
        ...
        var accumulation         = [],
            ...
            _flushAccumulator    = typeof flushAccumulator === 'function' ?
                function () {
                    accumulation.push(flushAccumulator.apply(arguments));
                } :
                function (chunk) {
...
//            'waiting'          : waiting,            //things waiting for accumulation
        ...
        if (toEmpty instanceof EventEmitter) {
            toEmpty.
                ...
                on(endEvent, function () {
                    waiting.forEach(tell.bind(null, tellEvent, accumulation, null));
                    accumulation = [];
                }).
                on('error', function (ex) {
                    waiting.forEach(tell.bind(null, tellEvent, accumulation, ex));
                    accumulation = [];
                });
