var util = require('util'),
    ...
    prepareToPump = require('./pump.js').prepareToPump,
    /**
     * shorthand for
    ...
    pump = function (event) {
        var self = prepareToPump(event, this, true);
...
exports.pump = function (event, eventEmitter) {
    if (eventEmitter instanceof EventEmitter) {
        return pump.call(eventEmitter, event);
    } else if (event instanceof EventEmitter) {
        return pump.call(event);
    } else {
        return prepareToPump(event);
    }
};
...
//will hook into the node eventloop and consolidate all stdout/stderr output into one stream separate from stdout/stderr
snake();
